...
export function createProgress({ noticeEl, getCode, getFiles, onChanged }) {
  const notice = createNotice(noticeEl);
  const { notify, dismiss } = notice;
  let timer = 0;
  ...
  /**
   * 정답을 처음 펼친 순간을 남긴다. 완료를 막지 않는다 — 이 도구에서 완료는 관문이 아니다.
   * 화면에는 그리지 않는다. 학습자를 벌주는 표시가 아니라 지표를 계산할 재료다(PRD §7.1).
   */
  const markRevealed = () => {
    if (!ctx.lessonId) return;
    saveStep(ctx.lessonId, ctx.stepIndex, { revealedAt: new Date().toISOString() });
  };
  ...
  return {
    ...
    dismiss,
    markRevealed,
    editorChanged: notice.editorChanged,
    resolveFiles,
    resetFile,
    flush,
    schedule,
