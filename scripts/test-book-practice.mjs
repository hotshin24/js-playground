...
for (const item of index.lessons.filter((item) => ['T2', 'T3'].includes(item.track))) {
  const lesson = JSON.parse(fs.readFileSync(new URL(`../lessons/${item.id}.json`, import.meta.url)));
  for (const step of lesson.steps.filter((step) => step.asserts?.some((spec) => spec.type === 'console'))) {
    for (const [code, expectedPass] of [
      [step.solutionCode, true],
      [step.code, false],
      ...
      assert.equal(
        events.length === plan.total && events.every((event) => event.status === 'pass'),
        expectedPass,
        item.id,
      );
      checks += 1;
...
console.log(`T2~T3 출력 정답·시작 코드·추가 출력 검사 ${checks}건 통과`);
